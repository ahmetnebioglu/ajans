...
export async function sendLoginOtpEmail(
  ...
  if (!apiKey) {
    console.warn(
      `[login-otp] RESEND_API_KEY yok — ${email} için kod: ${code}`
    );
  ...
  const from =
    process.env.RESEND_FROM || "Mercan ERP <onboarding@resend.dev>";

  const response = await resend.emails.send({
    from,
    to: [email],
    subject: "Mercan ERP — Giriş doğrulama kodu",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h1 style="font-size: 18px; font-weight: 800; text-transform: uppercase; color: #0f172a;">Mercan ERP</h1>
