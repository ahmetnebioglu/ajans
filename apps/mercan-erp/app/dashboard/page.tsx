...
export default async function DashboardPage() {
  ...
  const companyCount = await prisma.company.count({ where: companyFilter });
         ...
         <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-700 font-medium italic">
            ...
            <div className="relative overflow-hidden bg-white dark:bg-slate-900/50 rounded-[4px] p-8 text-slate-900 dark:text-white shadow-2xl border border-slate-200 dark:border-slate-900 transition-colors duration-500">
               ...
               <div className="relative z-10 space-y-3">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-[9px] font-black uppercase tracking-[0.4em] text-blue-600 dark:text-blue-400">Yönetim Paneli</span>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div className="bg-white dark:bg-slate-900/50 p-6 rounded-[4px] border border-slate-200 dark:border-slate-900 shadow-xl space-y-4 hover:border-blue-500 transition-all">
                  ...
                  <div className="space-y-1">
                     <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">Toplam Denetim Sayısı</p>
                     <p className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter italic">{reportCount}</p>
                  </div>
               </div>
               ...
               <div className={`p-6 rounded-[4px] border shadow-xl space-y-4 transition-all ${urgentCount > 0 ? "bg-rose-600 text-white border-rose-500 shadow-rose-600/20" : "bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800"}`}>
                  ...
                  <div className="space-y-1">
                     <p className={`text-[9px] font-black uppercase tracking-widest leading-none ${urgentCount > 0 ? "text-white/70" : "text-slate-400 dark:text-slate-500"}`}>Acil Aksiyon Bekleyenler</p>
                     <p className={`text-4xl font-black tracking-tighter italic ${urgentCount > 0 ? "text-white" : "text-rose-600 dark:text-rose-400"}`}>{urgentCount}</p>
                  </div>
               </div>
               ...
               <div className="bg-white dark:bg-slate-900/50 p-6 rounded-[4px] border border-slate-200 dark:border-slate-900 shadow-xl space-y-4">
                  ...
                  <div className="space-y-1">
                     <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">Son Güncelleme Tarihi</p>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-[4px] border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden">
               ...
               <div className="divide-y divide-slate-50 dark:divide-zinc-800">
                  {recentReports.slice(0, 3).map((report) => (
                     <div key={report.id} className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4 hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-all group">
                        <div className="flex items-center gap-4 text-center sm:text-left">
                           ...
                           <div>
                              <div className="font-black text-slate-900 dark:text-white uppercase tracking-tight text-base mb-0.5">{report.title}</div>
                              <div className="flex items-center justify-center sm:justify-start gap-3 text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                 <span className="flex items-center gap-1.5"><Activity size={10} className="text-blue-500" /> {report.category}</span>
                                 <span className="flex items-center gap-1.5"><Clock size={10} className="text-emerald-500" /> {new Date(report.createdAt).toLocaleDateString()}</span>
                              </div>
                           </div>
                        </div>
                        <Link 
           ...
           {/* FOOTER INFO */}
            <div className="p-8 bg-indigo-50/50 dark:bg-zinc-900/50 rounded-[4px] border border-indigo-100 dark:border-zinc-800 flex items-start gap-4 italic font-medium">
               ...
               <div className="space-y-1">
                  <h4 className="font-black text-slate-900 dark:text-white tracking-tight uppercase italic text-base leading-none">Müşteri Destek & Güvenlik</h4>
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-1000 font-medium italic">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-3 border-b border-slate-200 dark:border-zinc-800 pb-6">
         <div className="space-y-1">
            ...
            <p className="text-slate-500 dark:text-slate-500 font-bold uppercase tracking-widest text-[9px]">Mercan ERP Analitik Veri Merkezi</p>
         </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div key={s.name} className="bg-white dark:bg-slate-900/50 p-5 rounded-[4px] border border-slate-100 dark:border-slate-900 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
            ...
            <div className="flex items-center gap-3 relative z-10">
               ...
               <div>
                  <div className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-zinc-500 mb-0.5">{s.name}</div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className="bg-white dark:bg-zinc-900 p-6 rounded-[4px] border border-slate-200 dark:border-zinc-800 shadow-xl space-y-4 flex flex-col justify-between">
                  <div className="flex items-center justify-between">
                     <h3 className="text-[10px] font-black uppercase tracking-[0.2em] italic flex items-center gap-2 dark:text-slate-300 leading-none">
                        <TrendingUp className="text-blue-600 dark:text-blue-400" size={14} /> Statü Dağılımı
                     </h3>
                  </div>
               ...
               <div className="bg-white dark:bg-zinc-900 p-6 rounded-[4px] border border-slate-200 dark:border-zinc-800 shadow-xl space-y-4 flex flex-col justify-between">
                  <div className="flex items-center justify-between">
                     <h3 className="text-[10px] font-black uppercase tracking-[0.2em] italic flex items-center gap-2 dark:text-slate-300 leading-none">
                        <Activity className="text-indigo-600 dark:text-indigo-400" size={14} /> Aylık Trend
                     </h3>
                  </div>
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm italic">
                     <thead className="bg-slate-50 dark:bg-zinc-950/50 border-b border-slate-100 dark:border-zinc-800">
                        <tr>
                           <th className="p-4 text-[9px] font-black text-slate-400 dark:text-zinc-600 uppercase tracking-widest">Rapor & Firma</th>
                           <th className="p-4 text-[9px] font-black text-slate-400 dark:text-zinc-600 uppercase tracking-widest text-right">Zaman</th>
                        </tr>
                     </thead>
          ...
          <div className="space-y-4">
             ...
             <div className="bg-white dark:bg-slate-900/50 p-6 rounded-[4px] border border-slate-200 dark:border-slate-900 shadow-2xl space-y-6 flex flex-col">
                <div className="flex items-center justify-between">
                   ...
                   <span className="text-[7px] font-black bg-rose-50 dark:bg-rose-900/30 text-rose-500 dark:text-rose-400 px-1.5 py-0.5 rounded-[4px] tracking-[0.2em] uppercase shadow-sm border border-rose-100 dark:border-zinc-800">Audit Trace</span>
                </div>
