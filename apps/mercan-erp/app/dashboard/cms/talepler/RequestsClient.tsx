...
export default function RequestsClient({ contactRequests, referenceRequests }: RequestsClientProps) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-130px)] min-h-[600px] border border-slate-200 dark:border-white/5 rounded-[4px] overflow-hidden bg-white dark:bg-[#0F172A] w-full max-w-full shadow-sm">
      ...
      <div className="flex flex-col xl:flex-row items-center justify-between p-4 bg-slate-50 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/5 gap-4 overflow-hidden">
        ...
        <div className="flex items-center gap-4 w-full xl:w-auto">
          <div className="flex items-center gap-2 bg-white dark:bg-white/5 p-1 rounded-[4px] border border-slate-200 dark:border-white/5 overflow-x-auto no-scrollbar shrink-0">
          ...
          <button 
            ...
            className={`p-2.5 rounded-[4px] bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-500 hover:text-emerald-500 transition-all shadow-sm ${isRefreshing ? "text-emerald-500" : ""}`}
            title="Şimdi Yenile"
          >
        ...
        <div className="flex items-center gap-3 w-full xl:w-auto">
          {selectedRequest && (
            <div className="flex items-center gap-2 pr-4 border-r border-slate-200 dark:border-white/10 animate-in fade-in slide-in-from-right-4">
              <a 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white font-black text-[10px] uppercase italic tracking-widest rounded-[4px] shadow-lg shadow-emerald-600/20 hover:scale-105 transition-all shrink-0"
              >
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2.5 font-black text-[10px] uppercase italic tracking-widest rounded-[4px] transition-all shrink-0 border ${
          ...
          <div className="relative flex-1 xl:w-64">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-2.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[4px] text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all dark:text-white"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden w-full">
              ...
              <button
                ...
                className={`w-full text-left p-5 border-b border-slate-100 dark:border-white/5 transition-all relative group overflow-hidden ${
                  selectedRequest?.id === req.id 
                    ? "bg-white dark:bg-emerald-600/10 border-r-4 border-r-emerald-600 shadow-sm" 
              ...
              >
                <div className="flex justify-between items-start mb-1 gap-2">
                  <div className="flex items-center gap-2 overflow-hidden">
                    {!req.isRead && (
                      <div className="w-2 h-2 bg-emerald-600 rounded-full animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.6)] shrink-0"></div>
                    )}
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded shrink-0 ${
                      req.type === "contact" ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400" : "bg-purple-100 text-purple-600 dark:bg-purple-500/20 dark:text-purple-400"
                    }`}>
                ...
                } ${selectedRequest?.id === req.id ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
        ...
        <div className="hidden md:flex flex-1 flex-col bg-white dark:bg-[#0F172A] overflow-y-auto overflow-x-hidden">
          {selectedRequest ? (
            <div className="p-6 lg:p-8 space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              {/* Detail Header */}
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-100 dark:border-white/5 pb-8">
                <div className="flex items-center gap-5">
                  <div className={`w-16 h-16 rounded-[4px] flex items-center justify-center shrink-0 shadow-lg ${
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      ...
                      {selectedRequest.isVerified && (
                        <span className="px-3 py-1 bg-emerald-500 text-white text-[9px] font-black uppercase tracking-[0.2em] italic rounded-[4px] shadow-lg shadow-emerald-500/20">
                          DOĞRULANMIŞ
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-500 italic uppercase tracking-widest">
                       <span className="flex items-center gap-1.5 text-emerald-500"><Mail size={14} /> {selectedRequest.email}</span>
              ...
              <div className="space-y-6">
                ...
                <div className={`p-8 rounded-[4px] border leading-relaxed text-slate-700 dark:text-slate-300 font-medium italic shadow-inner ${
                  ...
                  {selectedRequest.type === "reference" ? (
                    <div className="space-y-4">
                      <p className="text-emerald-500 font-black uppercase tracking-widest text-[10px]">REFERANS TALEBİ AYRINTILARI</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div className="p-4 bg-white dark:bg-black/20 rounded-[4px] border border-slate-100 dark:border-white/5">
              ...
              <div className="pt-4">
                {!selectedRequest.isVerified && (
                  <div className="inline-flex items-center gap-2 text-amber-500 font-black text-[10px] uppercase tracking-widest italic bg-amber-500/10 px-4 py-2 rounded-[4px] border border-amber-500/20">
...
function FilterButton({ active, onClick, label, count }: { active: boolean; onClick: () => void; label: string; count: number }) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-[4px] text-[10px] font-black uppercase tracking-widest italic transition-all flex items-center gap-2 ${
        active 
