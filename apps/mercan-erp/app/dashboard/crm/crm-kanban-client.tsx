    ...
    >
      {COLUMNS.map((col) => (
        <KanbanColumn 
          key={col.id} 
          id={col.id}
          className="flex-1 min-w-[300px] max-w-[380px] h-full bg-zinc-900 border border-zinc-800 rounded-none flex flex-col shadow-lg relative overflow-hidden"
        >
          {/* Column Header - STICKY & SHARP */}
          <div className="sticky top-0 z-20 flex items-center justify-between border-b border-zinc-800 p-3 bg-zinc-900/95 backdrop-blur-sm shrink-0">
            <h3 className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-500 italic flex items-center gap-2">
              <span className="w-1.5 h-1.5 bg-teal-500 rounded-none animate-pulse" />
              {col.title}
            </h3>
            <span className="text-[9px] font-mono text-zinc-600 bg-black px-2 py-0.5 border border-zinc-800 rounded-none">
              {leads.filter(l => l.status === col.id).length}
            </span>
          </div>

          {/* Cards Area */}
          <div className="flex-1 p-2.5 space-y-2 overflow-y-auto min-h-0 scrollbar-hide">
            {leads.filter(l => l.status === col.id).map((lead) => (
              <LeadCard 
                key={lead.id} 
                id={lead.id} 
                item={lead}
                className="bg-black border border-zinc-800 rounded-none p-3 relative group cursor-grab active:cursor-grabbing hover:border-teal-500/50 transition-all shadow-md shrink-0"
                activeClassName="opacity-0"
              >
                {/* Neon Accent Line - SHARP */}
                <div className="absolute top-3 left-0 w-1 h-5 bg-zinc-800 group-hover:bg-teal-500 transition-colors" />

                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[7px] font-black uppercase tracking-widest text-teal-500 bg-teal-500/5 px-1.5 py-0.5 border border-teal-500/20 rounded-none">
                      {lead.source}
                    </span>
                  ...
                  <div>
                    <h4 className="text-[12px] font-black text-zinc-200 uppercase tracking-tighter italic flex items-center gap-1.5 leading-none">
                  ...
                  <div className="p-2 bg-zinc-950 border border-zinc-800/50 rounded-none">
