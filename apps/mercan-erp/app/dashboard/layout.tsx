...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans selection:bg-blue-100 dark:selection:bg-blue-900 italic font-medium transition-colors duration-500">
      {/* MOBILE HEADER */}
      <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-4 flex justify-between items-center shadow-sm sticky top-0 z-[60]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="text-slate-600 dark:text-slate-400 p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl"
          >
      ...
      >
        <div className="h-full flex flex-col p-6">
          ...
          <div className="flex items-center gap-3 mb-12 px-2 not-italic">
            <div className={`w-10 h-10 ${isCms ? "bg-emerald-600 shadow-emerald-500/50" : "bg-blue-600 shadow-blue-500/50"} rounded-xl flex items-center justify-center shadow-lg transform rotate-3 transition-colors duration-500`}>
              {isCms ? <Globe className="text-white" size={24} /> : <TrendingUp className="text-white" size={24} />}
            </div>
            <div className="text-slate-900 dark:text-white font-black text-xl tracking-tighter uppercase leading-none">
              Mercan
              <br />
              ...
              return (
                <Link
                  ...
                  className={`
                    ...
                    ${
                      isActive
                        ? `${themeActiveBg} ${themeActiveText} shadow-sm border ${themeActiveBorder}`
          ...
          <div className="mt-auto pt-6 border-t border-slate-100 relative" ref={profileRef}>
            ...
            {isProfileOpen && (
              <>
                <div className="absolute bottom-full left-0 w-full mb-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-[100] p-5 animate-in slide-in-from-bottom-4 duration-300 overflow-hidden leading-none italic">
                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-600 to-indigo-600" />

                  {/* ADMIN: IMPERSONATION TOOL */}
                  {realRole === "ADMIN" && (
                    <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 italic">
                  ...
                  <div className="space-y-1">
                    {[
                      ...
                      <div key={app.name} className="flex items-center gap-1 group">
                        <a
                          href={app.url}
                          className="flex-1 flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-all"
                        >
                          <div
                        ...
                        <button className="p-3 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all" title={app.action.label}>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col gap-1">
                    <Link
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl text-slate-600 hover:bg-slate-50 transition-all font-black text-[10px] uppercase tracking-widest"
                    >
                      <UserCircle size={16} /> Profil Ayarları
                    </Link>
                    <button
                      onClick={() => signOut({ callbackUrl: "/login" })}
                      className="w-full flex items-center gap-3 p-3 rounded-xl text-rose-500 hover:bg-rose-50 transition-all font-black text-[10px] uppercase tracking-widest"
                    >
      ...
      <main className={`flex-1 h-screen overflow-y-auto relative transition-colors duration-500 ${isCms ? "bg-emerald-50/20 dark:bg-slate-950" : "bg-slate-50/50 dark:bg-slate-950"}`}>
        ...
        <div className="hidden md:flex sticky top-0 z-[40] w-full p-6 justify-between items-center pointer-events-none">
           <div className="pointer-events-auto">
              {isCms && (
                <div className="flex items-center gap-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-1.5 pl-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl">
                   ...
                   <a 
                     href="/" 
                     target="_blank"
                     className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-600/20"
                   >
