...
export default function RichEditor({ content, onChange, label }: RichEditorProps) {
  ...
  const btnCls = (active?: boolean) =>
    `p-2 rounded-lg transition-all ${active ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-700 hover:text-white'}`;

  return (
    <div className="space-y-3">
      {label && <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{label}</label>}
      
      <div className="border border-slate-800 rounded-[2rem] overflow-hidden bg-slate-900/50 focus-within:ring-4 focus-within:ring-emerald-600/10 transition-all shadow-sm">
        {/* Toolbar */}
        <div className="flex flex-wrap items-center gap-1 p-3 bg-slate-800/50 border-b border-slate-800">
