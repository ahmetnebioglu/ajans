...
export default function ReportsTable({ 
  ...
  const SortHeader = ({ field, label }: { field: string, label: string }) => {
    ...
    return (
      <th onClick={() => handleSort(field)} className="p-6 cursor-pointer group hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors">
    ...
    <div className="relative">
                ...
                <thead className="bg-slate-50/50 dark:bg-slate-800/50 border-b dark:border-slate-800">
                    <tr>
                        <th className="p-6 w-10">
                           <input 
                            type="checkbox" 
                            className="w-4 h-4 rounded-md border-slate-300 dark:border-slate-700 bg-transparent text-blue-600 focus:ring-blue-500 cursor-pointer"
                        ...
                        <th className="p-6 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-right">Aksiyon</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {reports.map((report) => (
                    <tr key={report.id} className={`group transition-all duration-300 ${selectedIds.includes(report.id) ? "bg-blue-50/50 dark:bg-blue-900/20" : "hover:bg-slate-50/50 dark:hover:bg-slate-800/30"}`}>
                        ...
                        <td className="p-6 text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase italic opacity-70 group-hover:opacity-100">{report.company?.name}</td>
                        <td className="p-6">
                            <div className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest w-fit shadow-sm ${bgStatus(report.status)}`}>
                                {report.status?.replace("_", " ")}
                            </div>
                        </td>
                        <td className="p-6 text-[10px] font-bold text-slate-400 dark:text-slate-500 whitespace-nowrap italic"><Calendar size={12} className="inline mr-1" /> {new Date(report.createdAt).toLocaleDateString("tr-TR")}</td>
                        <td className="p-6 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <button onClick={() => setPreviewFile(report)} className="p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm"><Eye size={14} /></button>
                              <a href={report.fileUrl} target="_blank" className="p-2.5 bg-slate-900 dark:bg-slate-700 text-white rounded-lg hover:bg-blue-600 transition-all shadow-md"><ExternalLink size={14} /></a>
                           </div>
                        </td>
                    </tr>
                    ))}
           ...
           <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[300] animate-in slide-in-from-bottom-10 duration-500">
              <div className="bg-slate-900 text-white px-8 py-5 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-slate-800 flex items-center gap-8 backdrop-blur-md italic font-medium">
                 <div className="flex items-center gap-3 pr-8 border-r border-slate-700">
                    <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center font-black text-lg shadow-lg">{selectedIds.length}</div>
        ...
        {showBulkModal === "status" && (
           <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in">
              <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl p-10 space-y-8 animate-in zoom-in-95 border dark:border-slate-800">
                 ...
                 <div className="grid grid-cols-1 gap-2">
                    {["BEKLEMEDE", "AKSIYON_GEREKLI", "COZULDU"].map((s) => (
                       <button key={s} onClick={() => setBulkNewStatus(s)} className={`p-4 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest transition-all ${bulkNewStatus === s ? "bg-blue-50 dark:bg-blue-900/30 border-blue-600 dark:border-blue-400 text-blue-600 dark:text-blue-400 shadow-md" : "border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-600"}`}>
                          {s.replace("_", " ")}
                       </button>
                    ))}
                 </div>
                 <div className="flex gap-2 pt-4">
                    ...
                    <button onClick={handleBulkStatusUpdate} disabled={isBulkProcessing} className="flex-2 p-4 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2">
           ...
           <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in">
              <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl p-10 space-y-8 animate-in zoom-in-95 border-t-8 border-rose-600 dark:border-rose-500 shadow-[0_20px_50px_rgba(244,63,94,0.15)]">
                 ...
                 <div className="flex gap-2">
                    ...
                    <button onClick={handleBulkDelete} disabled={isBulkProcessing} className="flex-2 p-4 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2">
