...
export async function sendNewsletter(data: { projectSlug: string; subject: string; content: string }) {
  try {
    ...
    if (!resend) {
      return { success: false, error: "E-posta servisi yapılandırılamadı (API Key eksik)." };
    }
    ...
    if (subscribers.length === 0) {
      return { success: false, error: "Seçilen kriterlere uygun aktif abone bulunamadı." };
    }
    ...
    const { data: resendData, error } = await resend.emails.send({
      from: 'Mercan OSGB Bülten <newsletter@resend.dev>', // Custom domain bağlandığında güncellenmeli
      to: emails,
        ...
        <div style="font-family: sans-serif; max-width: 800px; margin: 0 auto; padding: 40px; border: 1px solid #e2e8f0; border-radius: 32px; background-color: #ffffff;">
          ...
          <div style="text-align: center;">
            <p style="color: #94a3b8; font-size: 11px; font-style: italic; margin-bottom: 24px;">
              Bu e-postayı Mercan OSGB bültenine abone olduğunuz için alıyorsunuz. 
