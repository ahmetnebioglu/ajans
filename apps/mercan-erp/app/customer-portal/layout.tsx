...
import { Layout, ConfigProvider, theme as antdTheme, Select, Avatar, Button, Card, Divider } from "antd";
import { signIn, useSession } from "next-auth/react";
import { useRouter } from "next/navigation";
import { useTheme } from "next-themes";
import CustomerSidebar from "../components/dashboard/CustomerSidebar";
import { Building2, Globe, Bell, ShieldCheck, ArrowRight, User } from "lucide-react";
import { getCustomerCompanies, setActiveCompany } from "../actions/customer-actions";
      ...
      <div className="max-w-md w-full space-y-8 animate-in fade-in zoom-in-95 duration-700">
        <div className="text-center space-y-4">
           ...
           <div className="space-y-1">
              <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">MÜŞTERİ PORTALI</h1>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em]">MERCAN UNIFIED OS | SECURE ACCESS</p>
           </div>
        </div>
        ...
        <Card className="shadow-2xl border-slate-200 dark:border-zinc-800 dark:bg-zinc-900 p-4 relative overflow-hidden group">
           ...
           <div className="space-y-6 relative z-10">
              <div className="space-y-1">
                 <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">PORTAL GİRİŞİ</h2>
                 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">Devam etmek için kayıtlı e-posta adresinizi girin.</p>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                 <div className="space-y-2">
                    ...
                    <input 
                      type="email" 
                      placeholder="customer@mercan.test"
              ...
              <div className="pt-4 text-center">
                 <p className="text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest leading-loose">
                    Bu alan sadece yetkili müşteriler içindir. <br /> Erişim sorunları için destek birimine başvurun.
                 </p>
              </div>
           </div>
        </Card>
...
export default function CustomerPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  if (session?.user && (session.user as any).role !== "CUSTOMER" && (session.user as any).role !== "ADMIN") {
    ...
    >
      <Layout style={{ minHeight: "100vh" }}>
        ...
        <Layout style={{ marginLeft: collapsed ? 80 : 240, transition: "all 0.2s" }}>
          <Header className={`px-6 flex items-center justify-between ${isDark ? 'bg-slate-900/50 border-white/5' : 'bg-white border-slate-200'} border-b h-16 sticky top-0 z-40 backdrop-blur-md`}>
            <div className="flex items-center gap-4">
               <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Aktif Firma:</span>
               <Select 
                 value={activeCompany} 
                 ...
                 options={companies.map(c => ({
                   value: c.id,
                   label: <span className="font-bold text-[11px] uppercase tracking-tighter italic">{c.name}</span>
                 }))}
            ...
            <div className="flex items-center gap-4">
               ...
               <div className="flex items-center gap-3">
                  <div className="text-right">
                     <div className="text-[10px] font-black uppercase text-slate-900 dark:text-white leading-none">{session?.user?.name}</div>
                     <div className="text-[8px] font-bold text-indigo-500 uppercase tracking-widest mt-0.5">Premium Üye</div>
                  </div>
                  <Avatar src={session?.user?.image} shape="square" className="bg-indigo-600" />
               </div>
            </div>
          </Header>
