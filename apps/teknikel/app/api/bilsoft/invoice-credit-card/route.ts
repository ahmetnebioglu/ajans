 ...
 * Kredi kartı ödeme yöntemi için kullanılır.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    // Global iskontoları (kupon, promosyon vb.) dağıt
    const globalIskonto =
      (parseFloat(order.couponDiscount as any) || 0) +
      (parseFloat(order.promotionDiscount as any) || 0);
    if (globalIskonto > 0 && faturaKalemleri.length > 0) {
      const totalExclBeforeGlobal = faturaKalemleri.reduce(
        (sum, it) => sum + it.topfiyat,
        0
      );
      if (totalExclBeforeGlobal > 0) {
        faturaKalemleri.forEach((it) => {
          const pay = (it.topfiyat / totalExclBeforeGlobal) * globalIskonto;
          it.iskontoTutar = parseFloat((it.iskontoTutar + pay).toFixed(2));
          it.topfiyat = parseFloat((it.topfiyat - pay).toFixed(2));
          it.kdvTutar = parseFloat(
            ((it.topfiyat * it.kdvOran) / 100).toFixed(2)
          );
          it.gtopfiyat = parseFloat((it.topfiyat + it.kdvTutar).toFixed(2));
        });
      }
    }

    // Fatura request body'sini oluştur
    const invoiceData = buildCreditCardInvoiceRequestBody(order, cariInfo, faturaKalemleri, tutarlar);

    console.log(`[invoice-credit-card] Bilsoft Fatura oluşturuluyor: ${order.id} - ${cariInfo.unvan}`);
    ...
    if (result.data?.id) {
      ...
      try {
        ...
        console.warn('[invoice-credit-card] İlk detay çekilemedi (normal bir durum):', (e as any).message);
      }
      ...
      if (needToAddItems) {
        ...
        if (itemsWithFatId.length > 0) {
          try {
            ...
            console.error('[invoice-credit-card] Error adding FatIsl:', (itemsError as any).message);
          }
        }
      }

      // Polling with exponential backoff - Fatura kalemlerinin Bilsoft'a düşmesini bekle
      try {
        const maxAttempts = 4;
        let attempt = 0;
        let delay = 1000;
    ...
    return NextResponse.json({
      success: true,
      message: 'Kredi kartı faturası başarıyla oluşturuldu',
      invoice: result.data,
    ...
    console.error('[invoice-credit-card] Error:', error);
    return NextResponse.json(
      {
        success: false,
...
async function prepareFaturaItems(items: any[], cariInfo: any, order: any, bilsoftToken: string) {
  ...
  for (const item of items) {
    try {
      ...
      console.error(`[invoice-credit-card] Ürün işleme hatası (${item.productSku}):`, error);
    }
  }
  ...
  try {
    ...
    console.warn('[invoice-credit-card] Dedupe hatası:', (dedupeErr as any).message);
  }
  ...
  try {
    ...
    if (shippingAmount > 0) {
      try {
        ...
        console.warn('[invoice-credit-card] Kargo stok kartı hatası:', (kargoErr as any).message);
      }
    }
  } catch (err) {
    console.warn('[invoice-credit-card] Kargo kalemi eklenirken hata:', (err as any).message);
  }
  ...
  try {
    ...
    if (installment > 1 && installmentCommission > 0) {
      try {
        ...
        console.warn('[invoice-credit-card] Taksit komisyonu hatası:', (commissionErr as any).message);
      }
    }
  } catch (err) {
    console.warn('[invoice-credit-card] Taksit komisyonu eklenirken hata:', (err as any).message);
  }
  ...
  try {
    ...
    console.warn('[invoice-credit-card] Yeniden sıralama hatası:', (reorderErr as any).message);
...
/**
 * Kredi kartı ödemeli fatura request body'sini oluştur
 */
function buildCreditCardInvoiceRequestBody(
  order: any,
  cariInfo: any,
  faturaKalemleri: any[],
  tutarlar: any
  ...
  // cariIsl - Cari işlemleri (alacak/borç kaydı) - Kredi kartı için hem borç hem alacak
  const cariIsl: any[] = [
    {
      id: 0,
  ...
  // stokIsl - Stok işlemleri (stok çıkışı)
  const stokIsl: any[] = [];
  ...
  // bankaIsl - Banka işlemleri (kredi kartı tahsilatı)
  const bankaIsl: any[] = [
    {
      id: 0,
      BankaId: 7,
  ...
  return {
    ...
    odenen: tutarlar.gtoplam,
    evrakNo: '',
    alinan: tutarlar.gtoplam,
