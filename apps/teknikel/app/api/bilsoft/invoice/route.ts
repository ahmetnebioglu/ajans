 ...
 * Normal ödeme yöntemleri için kullanılır.
 */
  ...
  try {
    ...
    for (const item of order.orderItems || []) {
      try {
        ...
        if (!stok) {
          console.warn(`[Invoice] Stok kartı bulunamadı: ${sku}`);
        ...
        console.error(`[Invoice] Kalem işleme hatası (${item.sku}):`, itemError);
        continue;
      }
    }
