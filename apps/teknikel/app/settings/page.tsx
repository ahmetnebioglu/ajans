  ...
  const integrationsContent = (
    <div className="space-y-4 py-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          { name: "Google Places API", status: "CONNECTED", icon: <ApiOutlined /> },
          { name: "NetGSM SMS API", status: "CONNECTED", icon: <ApiOutlined /> },
          { name: "Resend Email API", status: "DISCONNECTED", icon: <ApiOutlined /> },
          { name: "Prisma DB Engine", status: "CONNECTED", icon: <SettingOutlined /> },
        ].map((api) => (
      ...
      <Divider titlePlacement="left"><span className="text-[11px] font-bold text-slate-400 uppercase">API Anahtarları</span></Divider>
      <Form layout="vertical" size="small" className="max-w-md">
        <Form.Item label="Google API Key" name="googleKey">
          <Input.Password placeholder="AIza..." value="***************************" disabled />
        </Form.Item>
        <Form.Item label="NetGSM UserCode" name="netgsmUser">
          <Input placeholder="UserCode" value="8501234567" disabled />
        </Form.Item>
      </Form>
    </div>
  );
