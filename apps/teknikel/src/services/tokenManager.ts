...
export async function getValidToken(provider: string): Promise<string> {
  ...
  if (tokenRecord) {
    const isExpiring =
      tokenRecord.expiresAt.getTime() < now.getTime() + safetyMargin;
    ...
    console.log(
      `[TokenManager] ${provider} token süresi dolmak üzere veya dolmuş, yenileniyor...`,
    );
  } else {
    console.log(
      `[TokenManager] ${provider} için kayıtlı token bulunamadı, yeni alınıyor...`,
    );
  }
...
/**
 * Custom error type for IdeaSoft-related token errors.
 * Uses a simple `code` string so it can be inspected by callers.
 */
export class IdeasoftError extends Error {
  code: string;
  constructor(code: string, message: string) {
    super(message);
    this.code = code;
    this.name = "IdeasoftError";
  }
}

export function isIdeasoftRefreshMissing(err: any): boolean {
  if (!err) return false;
  if (err.code && err.code === "IDEASOFT_REFRESH_TOKEN_MISSING") return true;
  // Fallback to message inspection for older errors
  return (
    typeof err.message === "string" &&
    /refresh token bulunamadı/i.test(err.message)
  );
}
 ...
 */
async function refreshIdeasoftToken(
  ...
  if (!refreshToken) {
    throw new IdeasoftError(
      "IDEASOFT_REFRESH_TOKEN_MISSING",
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      "IdeaSoft OAuth yapılandırması eksik (client_id veya client_secret).",
    );
  }
  ...
  try {
    ...
    if (response.status !== 200) {
      console.error(
        "[TokenManager] IdeaSoft refresh token yanıtı:",
        response.status,
        response.data,
      );
      throw new IdeasoftError(
        "IDEASOFT_REFRESH_FAILED",
        `IdeaSoft token yenileme başarısız (HTTP ${response.status}). Refresh token geçersiz olabilir. Lütfen IdeaSoft entegrasyonunu yeniden başlatın.`,
      );
    }
    ...
    if (!access_token) {
      throw new IdeasoftError(
        "IDEASOFT_INVALID_RESPONSE",
        "IdeaSoft token yanıtında access_token eksik.",
      );
    }
    ...
    if (error.response) {
      console.error(
        "[TokenManager] IdeaSoft API hatası:",
...
export async function fetchWithIdeasoftRetry(
  ...
  if (response.status === 401 || response.status === 403) {
    ...
    try {
      // Yeni token al
      const newTokenData = await refreshIdeasoftToken(tokenRecord.refreshToken);
...
async function refreshBilsoftToken(): Promise<TokenData> {
  ...
  const credentials = {
    ...
    apiKullaniciSifre:
      dbConfig?.apiPassword || process.env.BILSOFT_API_PASSWORD,
