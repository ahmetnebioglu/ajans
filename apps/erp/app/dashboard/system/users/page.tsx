...
export default function UserManagementPage() {
  ...
  const handleToggleAccess = async (userId: string, workspaceId: string) => {
    ...
    const hasAccess = userAccessIds.includes(workspaceId);
    const res = await toggleExpertAccess(userId, workspaceId, !hasAccess);
    if (res.success) {
      if (!hasAccess) setUserAccessIds(prev => [...prev, workspaceId]);
      else setUserAccessIds(prev => prev.filter(id => id !== workspaceId));
      await loadData();
    }
                    ...
                    <div className="flex justify-end items-center gap-2">
                       {updatingId === user.id ? (
                         ...
                         >
                           ...
                           <option value="CUSTOMER">MÜŞTERİ (CUSTOMER)</option>
                           <option value="EXPERT">İSG UZMANI (EXPERT)</option>
                           <option value="HR_MANAGER">İK MÜDÜRÜ (HR)</option>
                           <option value="CLIENT">İSTEMCİ (CLIENT)</option>
