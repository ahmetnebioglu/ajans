  ...
  return (
    <div className="max-w-5xl mx-auto px-6 pb-12 space-y-6">
      <div className="flex justify-between items-center py-4">
        <div>
           <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">İSG <span className="text-emerald-600">Kütüphanesi</span></h1>
        ...
        <div className="flex gap-3">
          <button 
            ...
            className={`px-8 py-4 rounded-[4px] text-[11px] font-black uppercase tracking-widest flex items-center gap-3 transition-all shadow-sm active:scale-95 border ${
              isManagingCategories 
                ? "bg-slate-100 dark:bg-emerald-600/20 text-slate-500 dark:text-emerald-400 border-slate-200 dark:border-emerald-500/30" 
          ...
          <button 
            ...
            className={`px-8 py-4 rounded-[4px] text-[11px] font-black uppercase tracking-widest flex items-center gap-3 transition-all shadow-sm active:scale-95 ${
              isAdding ? "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400" : "bg-slate-900 dark:bg-emerald-600 text-white hover:bg-emerald-600 dark:hover:bg-emerald-700 shadow-emerald-600/20"
            }`}
          >
      ...
      {isManagingCategories && (
        <div className="p-6 border border-slate-200 dark:border-slate-800 rounded-[4px] bg-white dark:bg-slate-900 shadow-sm space-y-6 animate-in fade-in slide-in-from-top-4 duration-500 relative overflow-hidden">
           <div className="absolute top-0 left-0 w-full h-1 bg-slate-800 dark:bg-emerald-600" />
           ...
           <div className="flex gap-4">
              <input 
                ...
                className="flex-1 p-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[4px] text-sm font-bold text-slate-900 dark:text-white outline-none"
              />
              <button 
                onClick={handleAddCategory}
                disabled={loading || !newCatName}
                className="px-8 bg-emerald-600 text-white rounded-[4px] text-[11px] font-black uppercase tracking-widest disabled:opacity-50"
              >
                EKLE
              </button>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {categories.map(cat => (
                <div key={cat.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 rounded-[4px] group">
      ...
      {isAdding && (
        <div className="p-6 border border-slate-200 dark:border-slate-800 rounded-[4px] bg-white dark:bg-slate-900 shadow-sm space-y-6 animate-in fade-in slide-in-from-top-4 duration-500 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-8">
            <div className="space-y-3">
              ...
              <input 
                ...
                className="w-full p-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[4px] text-sm font-bold text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-emerald-600/5 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600"
              />
            </div>
            <div className="space-y-3">
              ...
              <select 
                ...
                className="w-full p-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[4px] text-sm font-bold text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-emerald-600/5 transition-all appearance-none cursor-pointer"
              >
            ...
            <div className="space-y-3">
              ...
              <div className="relative group">
                 ...
                 />
                 <div className="w-full p-6 bg-slate-50 dark:bg-slate-800/50 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[4px] flex flex-col items-center justify-center space-y-4 group-hover:border-emerald-600 group-hover:bg-emerald-50/30 dark:group-hover:bg-emerald-900/20 transition-all">
                    <Upload size={48} className="text-slate-400 dark:text-slate-600 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 group-hover:scale-110 transition-all" />
                    <div className="text-center">
                       ...
                       {newDoc.file && (
                         <div className="mt-4 px-4 py-1 bg-slate-900 dark:bg-emerald-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest inline-block">
                           TÜR: {newDoc.fileType}
                         </div>
                       )}
                    </div>
          ...
          <button 
            onClick={handleAdd} 
            disabled={loading}
            className="w-full py-4 bg-emerald-600 text-white rounded-[4px] text-[10px] font-black uppercase tracking-[0.2em] shadow-sm shadow-emerald-600/20 disabled:opacity-50 transition-all hover:bg-slate-900 dark:hover:bg-emerald-700 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3"
          >
      ...
      <div className="overflow-hidden border border-slate-100 dark:border-slate-800 rounded-[4px] bg-white dark:bg-slate-900 shadow-sm">
                ...
                <tr key={doc.id} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all group">
                  <td className="p-8">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 bg-slate-50 dark:bg-slate-800 rounded-[4px] flex items-center justify-center text-slate-400 dark:text-slate-600 group-hover:bg-emerald-600 group-hover:text-white dark:group-hover:text-white group-hover:rotate-6 transition-all duration-500 shadow-inner">
                        <FileText size={24} />
                      </div>
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                           <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">{doc.category?.name || "Kategorisiz"}</span>
                  ...
                  <td className="p-8 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <button 
                        ...
                        disabled={loading}
                        className={`px-6 py-2.5 rounded-[4px] text-[10px] font-black uppercase tracking-widest transition-all shadow-md ${
                          doc.isPublished 
                            ? 'bg-emerald-600 dark:bg-emerald-600 text-white shadow-emerald-600/20' 
                      ...
                      <button 
                        ...
                        disabled={loading} 
                        className="p-4 text-slate-200 dark:text-slate-700 hover:text-rose-600 dark:hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-[4px] transition-all"
                      >
                        <Trash2 size={20} />
                      </button>
                    </div>
