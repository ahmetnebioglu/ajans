...
export async function streamCrmInsights(userInput: string) {
  ...
  const result = await streamUI({
    ...
    prompt: userInput,
    text: ({ content }) => (
      <div className="text-sm text-slate-500 dark:text-zinc-400 italic p-4">
        {content}
      </div>
    ),
    tools: {
      getLeadsBySource: {
        description:
          "En çok lead gelen kaynakları analiz eder ve grafik olarak gösterir.",
        inputSchema: z.object({}),
        generate: async function* () {
          yield (
            <div className="p-4 text-[10px] font-black uppercase text-slate-400 dark:text-zinc-500 animate-pulse">
          ...
          const data = Object.entries(sourceMap).map(([name, count]) => ({
            name,
            count,
          }));
          ...
          return (
            <div className="bg-white dark:bg-black border border-slate-100 dark:border-zinc-800 p-6 rounded-none space-y-4 shadow-sm">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-teal-600 dark:text-teal-500 italic">
                Kaynak Analizi (Leads)
              </h3>
              <BarChart
                data={data}
                index="name"
      ...
      getLeadsByStatus: {
        description:
          "Leadlerin durum dağılımını (New, Contacted vb.) gösterir.",
        inputSchema: z.object({}),
        generate: async function* () {
          yield (
            <div className="p-4 text-[10px] font-black uppercase text-slate-400 dark:text-zinc-500 animate-pulse">
              Durumlar hesaplanıyor...
            </div>
          );
          ...
          return (
            <div className="bg-white dark:bg-black border border-slate-100 dark:border-zinc-800 p-6 rounded-none space-y-4 shadow-sm">
              ...
              <PieChart data={data} index="name" category="value" />
            </div>
          );
        },
      },
