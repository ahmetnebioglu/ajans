...
const MODULE_CONFIG = {
  ...
  cms: {
    ...
    items: [
      ...
      {
        key: "/dashboard/cms/homepage",
        icon: <Home size={18} />,
        label: "Ana Sayfa",
      },
      {
        key: "/dashboard/cms/about",
        icon: <Info size={18} />,
        label: "Kurumsal",
      },
      {
        key: "/dashboard/cms/services",
        icon: <LayoutIcon size={18} />,
        label: "Hizmetler",
      },
      {
        key: "/dashboard/cms/blog",
        icon: <Newspaper size={18} />,
        label: "Blog",
      },
      {
        key: "/dashboard/cms/settings",
  ...
  hr: {
    ...
    items: [
      ...
      {
        key: "/dashboard/hr/recruitment",
        icon: <UsersIcon size={18} />,
        label: "Aday Takip",
      },
      {
        key: "/dashboard/hr/jobs",
        icon: <Briefcase size={18} />,
        label: "İş İlanları",
      },
      {
        key: "/dashboard/hr/leaves",
  ...
  crm: {
    ...
    items: [
      ...
      {
        key: "/dashboard/crm/deals",
        icon: <LineChart size={18} />,
        label: "Satış Hattı",
      },
      {
        key: "/dashboard/crm/offers",
  ...
  const moduleSwitcherItems = [
    {
      ...
      children: [
        ...
        },
        {
          key: "/dashboard/hr",
          label: "HR",
          icon: <UsersIcon size={14} className="text-purple-500" />,
        },
        {
          key: "/dashboard/crm",
          label: "CRM",
          icon: <MessageSquare size={14} className="text-red-500" />,
        },
      ],
    },
    ...
    {
      key: "/dashboard/profile/sessions",
      ...
      children: [
        {
          ...
          label: (
            <div className="py-1 min-w-[200px]">
              <div className="font-bold text-blue-500 uppercase text-[9px] leading-none mb-1">
                Sistem Güncellemesi
              </div>
              <div className="text-[11px] text-slate-300 font-medium italic">
                Yeni audit log sayfası yayına alındı.
              </div>
              <div className="text-[8px] text-slate-500 mt-1 uppercase font-black">
                Az önce
              </div>
            </div>
          ),
        },
        {
          ...
          label: (
            <div className="py-1 min-w-[200px]">
              <div className="font-bold text-emerald-500 uppercase text-[9px] leading-none mb-1">
                Veri İhracı
              </div>
              <div className="text-[11px] text-slate-300 font-medium italic">
                Haftalık denetim raporu oluşturuldu.
              </div>
              <div className="text-[8px] text-slate-500 mt-1 uppercase font-black">
                2 saat önce
              </div>
    ...
    >
      <div className="h-full flex flex-col bg-transparent">
          ...
          >
            ...
            {!collapsed && (
              <div className="overflow-hidden animate-in fade-in duration-700">
                <div
                  className={`${isDark ? "text-white" : "text-slate-900"} font-black text-lg tracking-tighter uppercase leading-none italic whitespace-nowrap`}
                >
                  ERP{" "}
                  <span style={{ color: config.color }}>{config.name}</span>
                </div>
                <div
                  className={`text-[8px] ${isDark ? "text-slate-500" : "text-slate-400"} font-bold uppercase tracking-[0.3em] mt-1 italic`}
                >
                  Unified OS
                </div>
              </div>
        ...
        <div className="p-3 border-t border-white/5 bg-transparent">
          ...
          >
            {/* STATIC PROFILE INFO */}
            <div
              className={`flex items-center gap-2 ${collapsed ? "justify-center w-full" : "flex-1 min-w-0"}`}
            >
              <Avatar
                src={user?.image}
                shape="square"
              ...
              {!collapsed && (
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-[10px] font-black uppercase truncate tracking-tight leading-none mb-1 ${isDark ? "text-white" : "text-slate-900"}`}
                  >
                    {user?.name || "Kullanıcı"}
                  </div>
            ...
            <div
              className={`flex items-center gap-1 ${collapsed ? "flex-col mt-2" : ""}`}
            >
              {/* NOTIFICATIONS */}
              <Dropdown
              ...
              >
                <button
                  className={`w-8 h-8 rounded-[4px] ${isDark ? "bg-zinc-900 hover:bg-zinc-800 text-slate-500" : "bg-slate-50 hover:bg-slate-100 text-slate-500"} flex items-center justify-center transition-colors relative group`}
                >
                  <Bell size={16} />
                  <span
                    className={`absolute top-2 right-2 w-1.5 h-1.5 bg-red-500 rounded-full border border-${isDark ? "zinc-900" : "white"} group-hover:animate-ping`}
                  />
                </button>
              </Dropdown>
      ...
      <style jsx global>{`
        ...
        .premium-dropdown .ant-dropdown-menu {
          ...
          border: 1px solid
            ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.1)"} !important;
          padding: 8px !important;
          box-shadow: 0 20px 50px
            ${isDark ? "rgba(0,0,0,0.5)" : "rgba(0,0,0,0.05)"} !important;
        }
